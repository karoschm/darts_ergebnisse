...
**2.5 KORoundTab.js – Flackern von Default-Werten beim Rundenwechsel:** Gefunden (2026-08-21), noch offen. Beim Wechsel des KO-Runden-Tabs (neue `roundIndex`/`stageKey`-Props) initialisiert `KORoundTab.js` seinen lokalen State (`roundData = { matches: {} }`, `winLegs = 3`, `status = ""`) neu, bevor die neue `subscribeKnockoutRound`-Subscription den ersten echten Snapshot liefert. Dadurch werden kurz Default-Gewinnlegs (3) und leere Platzhalter-Matches statt der tatsächlichen Werte der Runde angezeigt, bis die Daten nachgeladen sind — sichtbares Flackern. Root Cause identisch zu 1.4 ("uneinheitliche/fehlende Ladezustände"), hier konkret durch fehlenden expliziten Ladezustand beim Tab-/Prop-Wechsel. Ansatz: pro `stageKey` einen `loading`-Flag führen und Inhalte erst nach dem ersten Snapshot rendern (z.B. Platzhalter/Spinner statt Default-Werten), oder — sauberer — die Runden-Docs im Elternkomponenten (`Running.js`) vorab/parallel laden statt pro Tab-Wechsel neu zu abonnieren. Sinnvoll direkt im Zuge von 2.2/2.3 nachzuziehen, da dieselben Subscriptions betroffen sind.
